...
console.log("\n2. Shared Omnio header hydrates consistently");
for (const p of ALL) {
  ...
  if (header.version !== "v3" || header.logo !== "omnio✦" || header.nav.join("|") !== expectedNav.join("|") || !header.search) {
    fail(`${p}: incomplete Omnio marketplace header ${JSON.stringify(header)}`);
  } else pass(`${p.padEnd(32)} shared Omnio header hydrated`);
}
